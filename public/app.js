  ...
  if ($('#id').val() == '') {
    ...
    $.ajax({
      ...
      success: function(data) {
        var accessManager = Twilio.AccessManager(data.token);
        conversationsClient = Twilio.Conversations.Client(accessManager, {logLevel: 'debug'});
        conversationsClient.listen().then(clientConnected, function(e) {
          console.log(e);
          tlog('Could not connect to Twilio: ' + e + ' </div>', true);
        });
      }
    });
  }
...
$(document).on('click', '.b-connect', function() {
  ...
  conversationsClient.inviteToConversation(user, options).then(conversationStarted, function(error) {
    tlog('We were unable to create the chat conversation with that user, try another online user.', error);
  });
});
